...
func runBenchmark(cfg *config, dataPath string, nShards int, t *testing.T) {
	...
	// Save latency and throughput to files for analysis
	saveLatenciesToFile(latencies, "latencies.txt")
	saveThroughputToFile(throughput, "throughput.txt")
}

func saveLatenciesToFile(latencies []time.Duration, filePath string) {
...
func saveThroughputToFile(throughput map[int]int, filePath string) {
	file, err := os.Create(filePath)
	if err != nil {
		fmt.Printf("Error creating file: %v\n", err)
		return
	...
	for sec, count := range throughput {
